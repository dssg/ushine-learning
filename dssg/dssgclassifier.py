...
class DssgVectorizerUnigramCount(DssgVectorizer):
    ...
    def fitTransform(self, messageList):
        ...
        for msg in messageList: 
            unigramList = self._unigramExtractor.extractUnigrams(msg['title'] + ' ' + msg['description']);
...
#            msg = messageList[i]['title'];
            row = {};
            unigramList = cache[i] if cache != None else \
...
class DssgCategoryClassifier(object):
    ...
    def train(cls, binaryClassifierTrainer, messageList, dssgVectorizerGenerator=None):
        ...
        """
        #- default vectorizer
        if (dssgVectorizerGenerator == None): dssgVectorizerGenerator = lambda: DssgVectorizerUnigramBySklearn()
        #--- Empty variables
        classifierDic = {};
        categoryList = [];
        ...
        for cat in categoryList:
            ...
            mvAcc = max([v, 1-v])
            logging.info('  nPos,nNeg = (%d / %d), majority vote acc = %.3f', nPos, nNeg, mvAcc);

            statDic = {'nPos':nPos, 'nNeg':nNeg, 'majority vote acc':mvAcc}

            #--- run cross validation to estimate performance
            try:
                cvResDic = dssgRunCV(binaryClassifierTrainer, dset, dssgVectorizerGenerator)
        ...
        logging.info('Overall aggregate prec = %.3f, recall = %.3f, F1 = %.3f', totPrecision, totRecall, sp.stats.hmean([totPrecision, totRecall]));
